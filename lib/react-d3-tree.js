...
var Tree = function (_React$Component) {
  ...
  function Tree(props) {
    ...
    _this.findMinNodeX = function () {
      return _this.findNodeExtremis(true, false);
    };

    _this.findMaxNodeX = function () {
      return _this.findNodeExtremis(true, true);
    };

    _this.findMinNodeY = function () {
      return _this.findNodeExtremis(false, false);
    };

    _this.findMaxNodeY = function () {
      return _this.findNodeExtremis(false, true);
    };
  ...
  _createClass(Tree, [{
    ...
    value: function bindZoomListener(props) {
      ...
      var rd3tSvgClassName = this.state.rd3tSvgClassName;
      var scaleExtent = props.scaleExtent;

      var svg = (0, _d.select)('.' + rd3tSvgClassName);
      var zoom = (0, _d.zoom)().scaleExtent([scaleExtent.min, scaleExtent.max]).on('zoom', function () {
        return _this2.zoom(props);
      });
      svg.call(zoom);
    ...
    key: 'zoom',
    value: function zoom(props) {
      // limit the horizontal panning to the x-extent of the tree's nodes
      var _event$transform = _d.event.transform,
      ...
      var k = _d.event.transform.k;

      var svgNode = svg.node();
      var svgWidth = svgNode.clientWidth;
      var minX = this.findMinNodeX();
      var maxX = this.findMaxNodeX();
      var shapeProps = this.props.nodeSvgShape.shapeProps;

      var halfNodeWidth = shapeProps.width / 2;
      ...
      var minY = this.findMinNodeY();
      var maxY = this.findMaxNodeY();
      var halfNodeHeight = shapeProps.height / 2;
      ...
      _d.event.transform.x = x;
      _d.event.transform.y = y;
      ...
      if (typeof onUpdate === 'function') {
        ...
        this.internalState.d3.translate = { x: x, y: y };
      }
    }
    ...
    key: 'findNodeExtremis',
    value: function findNodeExtremis(forX, findMax) {
      var nodes = this.internalState.nodes;

      var comparer = findMax ? Math.max : Math.min;
      var initialValue = findMax ? Number.MIN_SAFE_INTEGER : Number.MAX_SAFE_INTEGER;
      return nodes.reduce(function (acc, n) {
        return comparer(acc, forX ? n.y : n.x);
      }, initialValue);
    }
  }, {
    key: 'render',
...
var SvgTextElement = function (_React$PureComponent) {
  ...
  _createClass(SvgTextElement, [{
    key: 'render',
    value: function render() {
      var name = this.props.name.substr(0, 90);
      var _props = this.props,
      ...
      return _react2.default.createElement(
        ...
        _react2.default.createElement(
          'text',
          {
            className: 'nodeNameBase ' + (nodeData.nameLink ? ' nodeNameLink' : '') + ' ' + (name.length >= 80 ? ' superLongNodeName' : name.length >= 60 ? ' extraLongNodeName' : name.length >= 40 ? ' longNodeName' : ''),
